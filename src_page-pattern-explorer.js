...
(function(){
  ...
  function renderExplorer(){
    ...
    return pageHeader('Pattern explorer','Map the creative signals associated with performance across the active date range.')+'<div class="pattern-kicker"><span>SIGNAL MAP</span><span>Compare dimensions · validate before scaling</span></div>'+renderGlobalDateRange()+'<div class="explorer-note pattern-note"><div><strong>Association, not causation.</strong><span>Rankings summarize observed performance and evidence volume; use Creative Testing to validate a signal before scaling it.</span></div><span class="pattern-note-badge">5 dimensions</span></div><div class="pattern-dimension-grid">'+sections+'</div>;
  }
