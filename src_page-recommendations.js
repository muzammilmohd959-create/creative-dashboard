  ...
  PAGES.recommendations = {
    render: function () {
      ...
      var cards=evidence.map(function(i,idx){
        ...
        return '<article class="rec-card rec-card-premium"><div class="rec-glow"></div><div class="rec-top"><span class="insight-cat">'+escapeHtml(i.category)+' · '+escapeHtml(i.quality||'directional')+'</span><span class="rec-index">REC-'+String(idx+1).padStart(2,'0')+'</span></div><h2>'+escapeHtml(String(i.winner))+'</h2><p class="rec-proof">'+escapeHtml(proof)+'</p><div class="rec-evidence"><span>Baseline</span><strong>'+escapeHtml(i.metric==='cpa'?fmtCurrency2(i.baseline):i.metric==='ctr'?fmtPct(i.baseline):fmtX(i.baseline))+'</strong><span>Observed</span><strong>'+escapeHtml(i.metric==='cpa'?fmtCurrency2(i.value):i.metric==='ctr'?fmtPct(i.value):fmtX(i.value))+'</strong></div><p class="rec-action"><strong>Suggested test:</strong> Test more '+escapeHtml(String(i.winner))+' creative executions because this pattern is outperforming the comparison baseline.</p><button class="primary rec-create" data-winner="'+escapeHtml(String(i.winner))+'" data-proof="'+escapeHtml(proof)+'">Create creative brief →</button></article>';
      }).join('');
      return pageHeader('Recommendations','Turn proven signals into the next creative test — with evidence, context and a clear action.')+'<div class="rec-kicker"><span>ACTION LAYER</span><span>Evidence → brief → test</span></div>'+
        '<section class="recommendation-command"><div><span class="ops-eyebrow">CREATIVE ACTION ENGINE</span><h2>Know what to make next.</h2><p>Turn observed performance signals into the next creator brief. Every recommendation carries its baseline, observed result and evidence threshold so the action stays connected to the data.</p><div class="recommendation-flow"><span>OBSERVE</span><i>→</i><span>INTERPRET</span><i>→</i><strong>CREATE</strong><i>→</i><span>TEST</span></div></div><div class="recommendation-orb"><span>OPPORTUNITIES</span><strong>'+fmtNum(evidence.length)+'</strong><small>evidence-backed</small></div></section>'+
        '<div class="rec-intro rec-intro-premium"><div><span class="rec-intro-label">NEXT CREATIVE CYCLE</span><strong>'+fmtNum(evidence.length)+' evidence-backed opportunities</strong><span>Minimum spend and performance thresholds are applied by the intelligence engine.</span></div><div class="rec-intro-flow"><span>Signal</span><i>→</i><span>Brief</span><i>→</i><span>Test</span></div></div>'+
